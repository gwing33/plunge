...
class PlungeContext {
  constructor(endpoint) {
    ...
    if(endpoint.onChange) {
      this.addChangeListener(endpoint.onChange);
    }
    
    if(endpoint.onChanges) {
      this.addChangeListeners(endpoint.onChanges);
    }

    // Subscribe store before getting the data.
    Plunge.subscribe(x => {
      if(typeof this.onChange[x.valueInternal.uri] == 'function') {
        _.merge(this.data, x.data);
        this.onChange[x.valueInternal.uri](this, x.valueInternal);
      }
    }.bind(this));
   ...
   * @params {{uri:String, func:function}} args
   */
  addChangeListener() {
    if(typeof arguments[0] == 'function') {
      ...
      this.onChange[arguments[0].uri] = arguments[0].func;
    }
  }
